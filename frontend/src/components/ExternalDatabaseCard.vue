<template>
  <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <!-- EC selector -->
    <div v-if="groups.length > 0" class="mb-5 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div class="text-sm font-medium text-slate-900">Selected EC Number</div>
          <div class="mt-1 text-xs leading-5 text-slate-500">
            UniRef50 retrieval results are grouped by predicted EC labels.
          </div>
        </div>

        <select
          v-model="selectedEcNumber"
          class="w-full max-w-xs rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
        >
          <option
    ...
    <div v-if="paginatedItems.length > 0" class="space-y-5">
      ...
      >
        ...
        <div class="mb-4 flex flex-wrap items-start justify-between gap-4">
          <div class="min-w-0 flex-1">
            ...
            <div class="text-xs leading-5 text-slate-500">
              Rank #{{ item.rank }} · Retrieved under EC {{ item.ecNumber }}
            </div>
          </div>

          <div class="grid min-w-[250px] grid-cols-1 gap-2 sm:grid-cols-3">
        ...
        <!-- Aligned sequence -->
        <div class="mb-4">
          <div class="mb-2 text-[11px] font-medium uppercase tracking-[0.08em] text-slate-500">
        ...
        <div>
          ...
          <div v-if="expandedOriginalIds.has(item.id)" class="mt-3">
            <div class="mb-2 text-[11px] font-medium uppercase tracking-[0.08em] text-slate-500">
              Original Sequence
            </div>

            <PlainSequenceViewer
              :sequence="item.originalSequence"
              :visible-length="320"
            />
          </div>
        </div>
    ...
    >
      No UniRef50 retrieval results available for the selected EC number.
    </div>
  </section>
</template>
...
const getUniRefLink = (source) => {
  return `https://www.uniprot.org/uniref/${encodeURIComponent(source)}`
}
